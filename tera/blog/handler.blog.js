...
module.exports = function (app) {
  ...
  const update = async (req, res) => {
    ...
    const blog = await Model.findOne({ _id: blogId })
  ...
  const delAll = async (req, res) => {
    ...
    await Model.deleteMany({ author_id: userId })
      .then(() =>
        res.code(200).send({
          status: 'success',
  ...
  const read = async (req, res) => {
    const blogId = req.params.blog_id
    const blog = blogId.match(/^[0-9a-fA-F]{24}$/) ? await Model.findOne({ _id: blogId }) : await Model.findOne({ alternate_id: blogId })
